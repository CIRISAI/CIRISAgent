...
class TestPartnershipUtils:
    ...
    def test_create_partnership_task_no_reason(self, handler, mock_persistence):
        ...
        task = handler.create_partnership_task(user_id="user2", categories=["memory"], reason=None, channel_id=None)
    ...
    def test_create_partnership_task_empty_categories(self, handler, mock_persistence):
        ...
        task = handler.create_partnership_task(user_id="user3", categories=[], reason="Testing", channel_id="channel3")
    ...
    def test_create_partnership_task_context(self, handler, mock_persistence):
        ...
        task = handler.create_partnership_task(
            user_id="context_user", categories=["audit"], reason="Context test", channel_id="context_channel"
        )
    ...
    def test_check_task_outcome_rejected(self, handler, mock_persistence):
        ...
        mock_thought = Mock()
        mock_thought.final_action = Mock(action_type="REJECT", action_params={"reason": "Not appropriate"})
    ...
    def test_check_task_outcome_rejected_no_reason(self, handler, mock_persistence):
        ...
        mock_task = Mock(status=TaskStatus.FAILED)
        mock_thought = Mock()
        mock_thought.final_action = Mock(action_type="REJECT", action_params={})
    ...
    def test_check_task_outcome_deferred(self, handler, mock_persistence):
        ...
        mock_task = Mock(status=TaskStatus.FAILED)
        mock_thought = Mock()
        mock_thought.final_action = Mock(action_type="DEFER", action_params={"reason": "Need more info"})
    ...
    def test_check_task_outcome_deferred_no_reason(self, handler, mock_persistence):
        ...
        mock_task = Mock(status=TaskStatus.FAILED)
        mock_thought = Mock()
        mock_thought.final_action = Mock(action_type="DEFER", action_params={})
    ...
    def test_check_task_outcome_multiple_thoughts(self, handler, mock_persistence):
        ...
        mock_thought1 = Mock()
        mock_thought1.final_action = Mock(action_type="DEFER", action_params={"reason": "Old defer"})

        # Latest thought (will be used)
        mock_thought2 = Mock()
        mock_thought2.final_action = Mock(action_type="REJECT", action_params={"reason": "Final rejection"})
    ...
    def test_check_task_outcome_non_dict_params(self, handler, mock_persistence):
        ...
        mock_task = Mock(status=TaskStatus.FAILED)
        mock_thought = Mock()
        mock_thought.final_action = Mock(action_type="REJECT", action_params="Not a dict")  # Invalid params type
    ...
    def test_create_partnership_task_uuid_generation(self, handler, mock_persistence):
        ...
        task1 = handler.create_partnership_task(user_id="same_user", categories=["test"], reason=None, channel_id=None)

        task2 = handler.create_partnership_task(user_id="same_user", categories=["test"], reason=None, channel_id=None)
...
class TestPartnershipUtilsEdgeCases:
    ...
    def test_create_task_with_auth_service(self):
        ...
        mock_auth = Mock()
        handler = PartnershipRequestHandler(time_service=MockTimeService(), auth_service=mock_auth)

        assert handler.auth_service == mock_auth

        with patch("ciris_engine.logic.utils.consent.partnership_utils.persistence"):
            task = handler.create_partnership_task(
                user_id="auth_test", categories=["test"], reason=None, channel_id=None
            )
