...
- **No first-class `fidelity:lifecycle_application` event** — the EU-attested wire form requires fidelity "to declared purpose across the lifecycle". CIRIS has lifecycle observability (deployment manifests, version bumps, audit graph), but no module emits a typed `fidelity:lifecycle_application` attestation that ties the running agent back to its declared mission.
- **No `fidelity:algorithmic_disclosure` per Contribution** — ASEAN's densest sub-leaf (26 attestations) calls for proactive algorithmic disclosure to operators/users at every relevant decision; CIRIS exposes traces *on request* via VisibilityService but does not push proactive `fidelity:algorithmic_disclosure` envelopes.
- **No `fidelity:duty_bearer_obligation_to_fulfill_rights` attestation** — IEEE Ch11's wire form requires the agent to attest *fulfilment* of rights obligations (not just absence of harm). CIRIS has the audit ledger but no positive-fulfilment counter aggregated by rights category.
- **No automated `fidelity:explainability` SLA** — explainability today is best-effort (VisibilityService.explain_action returns `"Unable to explain action ... {error}"` on failure); there is no SLA or fallback guarantee.
...
- **MH `fidelity:epistemic_grounding`** — the agent's epistemic grounding is procedural (PDMA + four consciences) rather than substantive (tied to a specific canonical text the agent must remain faithful to). The polyglot Accord serves this role but the binding is policy, not enforced.
