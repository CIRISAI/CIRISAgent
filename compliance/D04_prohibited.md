...
- **No automated `prohibited:disinformation_at_scale` detector** — ASEAN's wire form references aggregate disinformation; CIRIS detects only per-thought attempts.
- **No `prohibited:deceptive_default_options` linter** — ASEAN cites this; CIRIS has no code that audits adapter manifests for dark-pattern defaults.
