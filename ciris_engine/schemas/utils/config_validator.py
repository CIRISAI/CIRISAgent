...
class ConfigData(BaseModel):
    ...
    llm_services: ConfigMapping = Field(default_factory=dict, description="LLM services config")
    database: ConfigMapping = Field(default_factory=dict, description="Database config")
    secrets: ConfigMapping = Field(default_factory=dict, description="Secrets config")
    additional_config: ConfigMapping = Field(default_factory=dict, description="Other configuration")


class LLMConfig(BaseModel):
    """LLM configuration."""

    openai: ConfigMapping = Field(default_factory=dict, description="OpenAI config")
    additional_providers: ConfigMapping = Field(default_factory=dict, description="Other LLM providers")
...
class DatabaseValidationConfig(BaseModel):
    ...
    additional_settings: ConfigMapping = Field(default_factory=dict, description="Other database settings")


class MaskedConfigResult(BaseModel):
    """Result of masking sensitive configuration values."""

    masked_config: ConfigMapping = Field(..., description="Configuration with masked sensitive values")
...
class NestedValueUpdate(BaseModel):
    ...
    target_object: ConfigMapping = Field(..., description="Object to update")
