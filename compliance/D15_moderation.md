...
**Substrate-enforced facet (inherited via CIRISServer).** Delegated, revocable moderation authority — enforced at the emit boundary — now ships inside the agent's own deployment via CIRISServer's integration of the substrate wheels, rather than remaining an upstream-pending item. The facet is independently verified against the *real published wheels* by CIRISConformance [`test_270_moderation_authority.py`](https://github.com/CIRISAI/CIRISConformance/blob/main/tests/test_270_moderation_authority.py) (CC 4.5.4/4.5.5 moderation duty): persist's `file_moderation` refuses a non-duty key with `federation_delegated_scope_unauthorized`; a community authority files; appoint → `is_named_moderator` → remove revokes, over the owner-bound (steward-bound) scoped-delegation chain — on both sqlite and postgres, across py3.10/3.12 on x86_64 + aarch64. D15 is one of the three controls the [CIRISConformance compliance coverage map](https://github.com/CIRISAI/CIRISConformance#compliance-coverage-map) marks fully substrate-gated (lane = substrate).
...
The agent ships broadcast, registration gate, kill-switch, audit chain, and conscience layer today, and the delegated/revocable moderation-authority facet is substrate-enforced via CIRISServer (conformance `test_270`, above). The remaining federation moderation closure — typed allegation envelopes and the corresponding NodeCore quorum review — is shared roadmap with the upstream substrate.
