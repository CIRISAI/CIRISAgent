...
async def complete_setup(setup: SetupCompleteRequest, request: Request) -> SuccessResponse[Dict[str, str]]:
    ...
    try:
        ...
        # attest, and cannot operate under the CIRIS trust model — so if
        # CIRISVerify cannot produce a signing key here, we must surface a
        # clear "unsupported platform" error and exit cleanly rather than
        # leaving a half-written .env behind or limping on without signing.
        try:
            ...
            msg = (
                f"Unsupported Platform for CIRISVerify: cannot initialize self-custody "
                f"signing key ({type(signing_err).__name__}: {signing_err}). "
                "The agent requires a functioning CIRISVerify runtime on this platform "
                "to sign audit entries and perform attestation. Setup has been rolled "
                "back; the agent will now shut down. Please install a supported "
                "CIRISVerify build for your platform and retry."
            )
            ...
            raise HTTPException(
                ...
                detail={
                    "code": "UNSUPPORTED_PLATFORM_CIRIS_VERIFY",
                    "message": msg,
                },
            )
