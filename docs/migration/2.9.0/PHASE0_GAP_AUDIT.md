...
- [x] [**CIRISPersist#66** — detected-secret store + process_incoming_text wiring.](https://github.com/CIRISAI/CIRISPersist/issues/66) `secrets_process_incoming_text` is a stub (always returns empty `refs`). `secrets_store_secret` is keyed-by-name and doesn't accept the agent's detected-secret metadata (pattern, sensitivity, context_hint, auto_decapsulate rules, source_message_id) with an agent-assigned UUID. Blocks Phase 2a.
