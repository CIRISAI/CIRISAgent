"""
Eightctl Tool Service for CIRIS.
...
class EightctlToolService:
    """
    Eightctl tool service providing skill-based guidance.
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self._call_count = 0
        logger.info("EightctlToolService initialized")

    async def start(self) -> None:
        """Start the service."""
        logger.info("EightctlToolService started")

    async def stop(self) -> None:
        """Stop the service."""
        logger.info("EightctlToolService stopped")
    ...
    def _build_tool_info(self) -> ToolInfo:
        ...
        return ToolInfo(
            ...
            category="skill",
            when_to_use="""When you need to control eight sleep pods (status, temperature, alarms, schedules).""",
            requirements=ToolRequirements(
                binaries=[
                    BinaryRequirement(name="eightctl"),
                ],
            ),
            install_steps=[
                InstallStep(id="go", kind="go", label="Install eightctl (go)", provides_binaries=["eightctl"]),
            ],
            documentation=ToolDocumentation(
                quick_start="Use `eightctl` for Eight Sleep pod control. Requires auth.",
                detailed_instructions="""# eightctl\n\nUse `eightctl` for Eight Sleep pod control. Requires auth.\n\nAuth\n- Config: `~/.config/eightctl/config.yaml`\n- Env: `EIGHTCTL_EMAIL`, `EIGHTCTL_PASSWORD`\n\nQuick start\n- `eightctl status`\n- `eightctl on|off`\n- `eightctl temp 20`\n\nCommon tasks\n- Alarms: `eightctl alarm list|create|dismiss`\n- Schedules: `eightctl schedule list|create|update`\n- Audio: `eightctl audio state|play|pause`\n- Base: `eightctl base info|angle`\n\nNotes\n- API is unofficial and rate-limited; avoid repeated logins.\n- Confirm before changing temperature or alarms.""",
                examples=[],
                gotchas=[],
            ...
            tags=["skill", "clawdbot", "eightctl", "cli"],
            version="1.0.0",
        )
