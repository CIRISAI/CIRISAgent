...
def test_db_path(tmp_path: Path) -> str:
    ...
    db_file = tmp_path / "test_tsdb_edge.db"
    db_path = str(db_file)
...
@pytest.fixture
def edge_manager(test_db_path: str) -> EdgeManager:
    """Create an edge manager with isolated database for testing."""
    return EdgeManager(db_path=test_db_path)


@pytest.fixture
def test_db_connection(test_db_path: str):
...
class TestTemporalEdgeCreation:
    ...
    @pytest.mark.asyncio
    async def test_daily_summary_temporal_edges(self, edge_manager, test_db_path):
        """Test temporal edges between daily summaries."""
        # Use separate connections for setup and edge creation to avoid WAL lock conflicts
        conn = sqlite3.connect(test_db_path)
        conn.row_factory = sqlite3.Row
        cursor = conn.cursor()
        ...
        conn.commit()
        conn.close()  # Close connection before edge_manager creates edges

        # Now create edges using edge_manager (which will create its own connection)
        ...
        # Reopen connection to verify results
        conn = sqlite3.connect(test_db_path)
        conn.row_factory = sqlite3.Row
        cursor = conn.cursor()
    ...
    @pytest.mark.asyncio
    async def test_gap_in_temporal_chain(self, edge_manager, test_db_path):
        """Test temporal edges when there's a gap in the chain."""
        # Use separate connections to avoid WAL lock conflicts
        conn = sqlite3.connect(test_db_path)
        conn.row_factory = sqlite3.Row
        cursor = conn.cursor()
        ...
        conn.commit()
        conn.close()  # Close before edge creation
...
class TestSameDayEdgeCreation:
    ...
    @pytest.mark.asyncio
    async def test_same_day_cross_type_edges(self, edge_manager, test_db_path, test_db_connection):
        ...
        # Create same-day edges using a service with the test database
        from ciris_engine.logic.services.graph.tsdb_consolidation.service import TSDBConsolidationService

        service = TSDBConsolidationService(db_path=test_db_path)
    ...
    @pytest.mark.asyncio
    async def test_partial_types_same_day(self, edge_manager, test_db_connection):
...
class TestDuplicatePrevention:
    ...
    @pytest.mark.asyncio
    async def test_duplicate_edge_prevention(self, edge_manager, test_db_connection):
    ...
    @pytest.mark.asyncio
    async def test_update_next_period_edges(self, edge_manager, test_db_path):
        ...
        # Delete old edges to rewire the chain
        cursor.execute(
            """
        ...
        conn.commit()
        conn.close()  # Close before creating new edges

        # Create new edges for day 2
        edge_manager.create_temporal_edges(summary2, summary1.id)

        # Update day3 to point back to day2
        conn = sqlite3.connect(test_db_path)
        conn.row_factory = sqlite3.Row
        cursor = conn.cursor()
...
class TestEdgeAttributes:
    ...
    @pytest.mark.asyncio
    async def test_edge_attributes_json(self, edge_manager, test_db_connection):
    ...
    @pytest.mark.asyncio
    async def test_days_apart_attribute(self, edge_manager, test_db_connection):
...
class TestCleanupOrphanedEdges:
    ...
    @pytest.mark.asyncio
    async def test_cleanup_orphaned_edges(self, edge_manager, test_db_connection):
    ...
    @pytest.mark.asyncio
    async def test_cleanup_with_database_locked_error(self, test_db_path, test_db_connection):
        """Test that cleanup retries when database is locked."""
        from contextlib import contextmanager

        # Create a fresh edge manager for this test (we'll mock the connection)
        edge_manager = EdgeManager(db_path=test_db_path)
        ...
        def mock_conn_with_retry(db_path=None, **kwargs):
            ...
            # Success on 3rd attempt - use actual connection to test db
            conn = sqlite3.connect(test_db_path)
            conn.row_factory = sqlite3.Row
            try:
                yield conn
    ...
    @pytest.mark.asyncio
    async def test_cleanup_max_retries_exceeded(self, test_db_path):
    ...
    @pytest.mark.asyncio
    async def test_cleanup_exponential_backoff_timing(self, test_db_path, test_db_connection):
        ...
        def mock_conn_track_timing(db_path=None, **kwargs):
            ...
            if len(retry_times) < 3:
                raise Exception("database is locked")
            # Success on 3rd attempt - use actual connection
            conn = sqlite3.connect(test_db_path)
            conn.row_factory = sqlite3.Row
            try:
                yield conn
    ...
    @pytest.mark.asyncio
    async def test_cleanup_non_locking_error_no_retry(self, test_db_path):
    ...
    @pytest.mark.asyncio
    async def test_cleanup_retry_success_after_one_failure(self, test_db_path, test_db_connection):
...
class TestDailyConsolidationEdgeCreation:
    ...
    def test_get_previous_summary_id_daily(self, edge_manager, test_db_connection):
    ...
    def test_get_previous_summary_id_regular(self, edge_manager, test_db_connection):
    ...
    def test_get_previous_summary_id_with_multi_day_gap(self, edge_manager, test_db_connection):
    ...
    def test_daily_temporal_edge_creation_with_previous(self, edge_manager, test_db_connection):
    ...
    def test_daily_temporal_edge_creation_first_summary(self, edge_manager, test_db_connection):
...
class TestDailyConsolidationService:
    ...
    def test_create_daily_summary_edges_correct_id_parsing(
        self, mock_memory_bus, mock_time_service, test_db_path, test_db_connection
    ):
    ...
    def test_daily_summary_edge_validation(self, edge_manager, test_db_connection):
