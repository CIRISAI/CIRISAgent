...
class MCPBusBindingSchema(BaseModel):
    ...
    capability_filter: List[str] = Field(
        default_factory=list, description="Filter to specific capabilities"
    )
...
class MCPSecurityConfigSchema(BaseModel):
    ...
    permission_level: MCPPermissionLevel = Field(
        MCPPermissionLevel.SANDBOXED, description="Permission level"
    )
    allowed_tools: List[str] = Field(
        default_factory=list, description="Whitelist of allowed tools"
    )
    blocked_tools: List[str] = Field(
        default_factory=list, description="Blacklist of blocked tools"
    )
...
class MCPServerConfigSchema(BaseModel):
    ...
    transport: MCPTransportType = Field(
        MCPTransportType.STDIO, description="Transport type"
    )
    ...
    bus_bindings: List[MCPBusBindingSchema] = Field(
        default_factory=list, description="Bus bindings"
    )
    security: MCPSecurityConfigSchema = Field(
        default_factory=MCPSecurityConfigSchema, description="Security config"
    )
