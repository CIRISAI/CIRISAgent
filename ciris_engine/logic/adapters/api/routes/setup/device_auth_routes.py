...
async def connect_node_status(device_code: str, portal_url: str) -> SuccessResponse[ConnectNodeStatusResponse]:
    ...
    logger.info("[connect-node/status] Calling _register_self_custody_key(device_code=%s, portal_url=%s)", device_code, portal_url)
    key_id = await _register_self_custody_key(device_code, portal_url)
