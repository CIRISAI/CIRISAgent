...
class MCPSecurityManager:
    ...
    async def check_version(
        self, server_id: str, current_version: str
    ) -> Tuple[bool, Optional[SecurityViolation]]:
        ...
        if config.pin_version and config.pin_version != current_version:
            if not config.allow_version_updates:
                violation = SecurityViolation(
                    ...
                    tool_name=None,
                    description=(
                        f"Server version mismatch: expected {config.pin_version}, got {current_version}"
                    ),
                    severity="high",
    ...
    async def _record_violation(self, violation: SecurityViolation) -> None:
        ...
        # Log the violation
        log_method = logger.critical if violation.severity == "critical" else (
            logger.error if violation.severity == "high" else (
                logger.warning if violation.severity == "medium" else logger.info
            )
        )
        log_method(
