...
- LensCore F-3 detector family (cross-trace integrity correlation analysis) is not in this repo. The substrate substitution trajectory routes integrity detection into LensCore at step 3 (LensCore Rust crate swap). Today the agent emits integrity dimensions per-trace; the federation aggregates them.
- No automated `attestation:l5` ratification flow yet — L5 requires both `audit_ok` and `play_integrity_ok`, but desktop/server deployments leave `play_integrity_ok=false` by design (`ciris_engine/schemas/services/attestation.py:40`). The ladder is mobile-anchored; server-side L5 needs a TPM-quote equivalent that isn't wired.
- `accord_public_keys` registration (`service.py:1790`) is one-way — there's no rotation-detection alert path beyond the persist-side "collision with differing content" exception. Key rotation requires operator approval but doesn't surface to the integrity dashboard.
