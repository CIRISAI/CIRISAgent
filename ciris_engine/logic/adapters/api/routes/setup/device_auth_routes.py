...
async def connect_node_status(device_code: str, portal_url: str) -> SuccessResponse[ConnectNodeStatusResponse]:
    ...
    logger.info("[connect-node/status] Poll request: device_code=%s..., portal_url=%s", device_code[:16] if device_code else "None", portal_url)
    ...
    logger.info("[connect-node/status] Calling _register_self_custody_key(device_code=%s, portal_url=%s, challenge=%s)",
                device_code, portal_url, registration_challenge[:16] + "..." if registration_challenge else "None")
