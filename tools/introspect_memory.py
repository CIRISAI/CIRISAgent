...
async def _run_message_load(
    ...
    return success, failed, time.time() - start, task_ids
...
def main(
    ...
    try:
        if adapters == "api" and messages > 0:
            ...
            load_start = datetime.now(timezone.utc).isoformat()
            success, failed, elapsed, task_ids = asyncio.run(
                _run_message_load(base_url, token, messages, concurrency, process_tasks=verify_audit)
            )
            print(f"  Message load complete: {success}/{messages} in {elapsed:.1f}s (failed={failed})")
            if verify_audit and success > 0:
                verified, task_complete_count, speak_count = _verify_task_processing(task_ids, success, load_start)
                if verified:
                    print(
