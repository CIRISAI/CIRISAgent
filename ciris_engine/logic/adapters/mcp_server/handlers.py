...
class MCPResourceHandler:
    ...
    async def handle_list_resources(self, request_id: Any) -> MCPMessage:
        ...
        """
        resources = [
            r for r in self._resources_cache.values()
            if self._should_expose_resource(r.uri)
        ]
...
class MCPPromptHandler:
    ...
    def _register_default_prompts(self) -> None:
        ...
        from ciris_engine.logic.adapters.mcp_common.schemas import MCPPromptArgument
    ...
    async def handle_list_prompts(self, request_id: Any) -> MCPMessage:
        ...
        """
        prompts = [
            p for p in self._prompts_cache.values()
            if self._should_expose_prompt(p.name)
        ]
