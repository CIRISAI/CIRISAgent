...
class TestUserNodeCorruptionFix:
    ...
    def mock_task(self):
        ...
        return Task(
            ...
            context=TaskContext(correlation_id="test_correlation", user_id="537080239679864862"),
        )
    ...
    async def test_fix_template_placeholder_in_last_seen(
        ...
        with patch("ciris_engine.logic.context.system_snapshot.logger") as mock_logger, patch(
            ...
            mock_persistence.get_db_connection.return_value.__enter__.return_value.cursor.return_value.fetchone.return_value = (
                None
            )
    ...
    async def test_fix_unparseable_last_seen(
        ...
        corrupted_attrs = {"username": "test_user2", "last_seen": "not-a-date", "email": "test2@example.com"}
    ...
    async def test_fix_continues_on_update_failure(
        ...
        with patch("ciris_engine.logic.context.system_snapshot.logger") as mock_logger, patch(
            ...
            assert any(
                "Failed to fix corrupted node" in str(call) for call in mock_logger.error.call_args_list
    ...
    async def test_valid_last_seen_not_modified(
        ...
        valid_attrs = {"username": "test_user5", "last_seen": valid_timestamp, "email": "test5@example.com"}
        ...
        with patch("ciris_engine.logic.context.system_snapshot.logger") as mock_logger, patch(
            ...
            assert not any(
                "INVALID DATA" in str(call) for call in mock_logger.error.call_args_list
