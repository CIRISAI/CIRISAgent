...
async def complete_setup(setup: SetupCompleteRequest, request: Request) -> SuccessResponse[Dict[str, str]]:
    ...
    try:
        ...
        # SELF-CUSTODY KEY (FSD-002): the agent's signing key is generated by
        # CIRISVerify at startup (TPM-protected) and registered with Portal.
        # A CIRIS agent without a working signing key cannot audit, cannot
        # attest, and cannot operate under the CIRIS trust model.
        #
        # ``get_unified_signing_key()`` only ever produces one of two terminal
        # outcomes (see ``CIRISVerifySigner.initialize``):
        #
        #   * SUCCESS — a usable Ed25519 key is loaded. We proceed.
        #   * FAILURE — verify's FFI is genuinely unusable on this device.
        #
        # ``AttestationInProgressError`` is NOT a third outcome — the signer
        # waits patiently while verify holds its attestation lock and only
        # raises when verify itself declares the capability dead. So the catch
        # block here always indicates a real "verify cannot sign on this
        # platform" condition, not a transient.
        try:
            ...
            # client's "Technical details:" line shows the real cause (e.g.
            # missing FFI symbol, native crash) instead of an opaque code.
            # User-facing copy lives in the localized Kotlin/Swift strings.
            msg = (
                f"CIRISVerify signing capability unavailable on this device "
                f"({type(signing_err).__name__}: {signing_err}). "
                "The agent cannot sign audit entries without a functioning "
                "CIRISVerify FFI runtime. Setup has been rolled back; the "
            ...
            if runtime is not None:
                try:
                    runtime.request_shutdown(
                        "CIRISVerify signing capability unavailable: signing key initialization failed"
                    )
