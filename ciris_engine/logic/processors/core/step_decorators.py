    ...
    def decorator(func: F) -> F:
        ...
        async def wrapper(self, thought_item, *args, **kwargs):
            ...
            if not time_service or not hasattr(time_service, 'now'):
                raise RuntimeError(f"Critical error: No time service available for step {step.value} on thought {thought_id}")
...
def get_paused_thoughts() -> Dict[str, str]:
    ...
    return {
        thought_id: "paused_awaiting_resume" 
        for thought_id in _paused_thoughts.keys()
    }