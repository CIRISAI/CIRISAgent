"""
NanoPdf Tool Service for CIRIS.
...
class NanoPdfToolService:
    """
    NanoPdf tool service providing skill-based guidance.
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self._call_count = 0
        logger.info("NanoPdfToolService initialized")

    async def start(self) -> None:
        """Start the service."""
        logger.info("NanoPdfToolService started")

    async def stop(self) -> None:
        """Stop the service."""
        logger.info("NanoPdfToolService stopped")
    ...
    def _build_tool_info(self) -> ToolInfo:
        ...
        return ToolInfo(
            ...
            category="skill",
            when_to_use="""When you need to edit pdfs with natural-language instructions using the nano-pdf cli.""",
            requirements=ToolRequirements(
                binaries=[
                    BinaryRequirement(name="nano-pdf"),
                ],
            ),
            install_steps=[
                InstallStep(
                    ...
                    package="nano-pdf",
                    provides_binaries=["nano-pdf"],
                ),
            ],
            documentation=ToolDocumentation(
                quick_start="Use `nano-pdf` to apply edits to a specific page in a PDF using a natural-language instruction.",
                detailed_instructions="""# nano-pdf\n\nUse `nano-pdf` to apply edits to a specific page in a PDF using a natural-language instruction.\n\n## Quick start\n\n```bash\nnano-pdf edit deck.pdf 1 \"Change the title to 'Q3 Results' and fix the typo in the subtitle\"\n```\n\nNotes:\n- Page numbers are 0-based or 1-based depending on the tool’s version/config; if the result looks off by one, retry with the other.\n- Always sanity-check the output PDF before sending it out.""",
                examples=[],
                gotchas=[],
            ...
            tags=["skill", "clawdbot", "nano_pdf", "cli"],
            version="1.0.0",
        )
