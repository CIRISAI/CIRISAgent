...
def setup_ios_environment() -> Path:
    ...
    # SSL certificates: iOS embedded Python doesn't inherit the system CA store.
    # Point all Python SSL (aiohttp, urllib3, etc.) at certifi's Mozilla CA bundle.
    try:
        import certifi
        os.environ.setdefault("SSL_CERT_FILE", certifi.where())
        os.environ.setdefault("REQUESTS_CA_BUNDLE", certifi.where())
    except ImportError:
        pass  # certifi not available — SSL will fail for HTTPS calls
