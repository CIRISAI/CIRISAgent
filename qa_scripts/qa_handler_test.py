...
special mock LLM commands to force specific handler execution.
"""
...
class HandlerTestSuite:
    ...
    def get_audit_log(self, limit: int = 10) -> List[Dict]:
        ...
        response = requests.get(
            f"{BASE_URL}/v1/audit/entries?limit={limit}",
            headers={"Authorization": f"Bearer {self.token}"}
        )
    ...
    def test_handler(self, handler_name: str, command: str, description: str) -> bool:
        ...
        if "error" in result:
            ...
            handler_executed = any(
                handler_name.lower() in str(e).lower() 
    ...
    def run_all_tests(self):
        ...
        # Test each handler using mock LLM force commands
        handlers = [
            # Basic handlers
            ("SPEAK", "$SPEAK Hello from QA test!", "Send a message"),
            ("OBSERVE", "$OBSERVE", "Observe channel messages"),
            
            # Memory handlers
            ("MEMORIZE", "$MEMORIZE test_qa_node CONCEPT test_data", "Store memory"),
            ("RECALL", "$RECALL test_qa_node", "Recall memory"),
            ("FORGET", "$FORGET test_qa_node", "Forget memory"),
            
            # Wisdom handlers  
            ("DEFERRAL", "$DEFER Need guidance on test", "Request wisdom"),
            ("APPROVAL", "$DEFER test_approval", "Request approval"),
            
            # Tool/Task handlers
            ("EXECUTE", "$TOOL test_tool test_params", "Execute tool"),
            ("ROUTING", "$TOOL routing test_route", "Route to service"),
            ("WAITING", "$PONDER 5", "Wait for event"),
        ]
        ...
        handler_events = [
            ...
            if any(h.lower() in str(e).lower() for h in self.test_results.keys())
        ]
