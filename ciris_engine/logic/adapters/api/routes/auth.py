...
def _authorize_private_redirect(redirect_hostname: str, redirect_domain: str, redirect_uri: str) -> Optional[str]:
    """#846: authorize a non-loopback private host (LAN/.local/link-local).

    Trusted only via explicit opt-in or an allowlist entry — never blanket —
    so a crafted login link can't exfiltrate a bearer token to a LAN host.
    """
    if OAUTH_ALLOW_PRIVATE_REDIRECTS:
        logger.debug(f"Allowing redirect to private network host (opt-in): {redirect_hostname}")
        return redirect_uri
    if _is_domain_allowed(redirect_domain, _get_allowed_redirect_domains()):
...
def _authorize_redirect_host(
    redirect_hostname: str, redirect_uri: str, is_loopback: bool, is_private: bool
) -> Optional[str]:
    """Authorize an absolute redirect by host class (loopback / private / public)."""
    redirect_domain = redirect_hostname.lower()

    # #846: Loopback (same machine) stays trusted by default — the Home Assistant
    # IndieAuth path: HTTP redirect back to a localhost OAuth endpoint we run. A
    # token sent to 127.0.0.1 cannot leave the host.
    if is_loopback:
        logger.debug(f"Allowing redirect to loopback host: {redirect_hostname}")
        return redirect_uri

    if is_private:
        return _authorize_private_redirect(redirect_hostname, redirect_domain, redirect_uri)

    # Public host: must be on the allowlist.
...
def validate_redirect_uri(redirect_uri: Optional[str]) -> Optional[str]:
    ...
    try:
        ...
        return _authorize_redirect_host(redirect_hostname, redirect_uri, is_loopback, is_private)
...
async def _verify_google_id_token(id_token: str) -> Dict[str, Optional[str]]:
    ...
    try:
        ...
        # FAIL CLOSED (#999). This used to fall back to _decode_google_jwt_locally,
        # which does NOT verify the token signature — its own docstring said so.
        #
        ...
        # provenance, and HTTP cannot establish it: this is a public endpoint, and
        # anyone can POST a JWT. An attacker who can make the outbound call to
        # oauth2.googleapis.com time out — a hostile network, a DNS hold, a
        # sufficiently slow link — turned a FORGED id_token into a real CIRIS
        # session, because the fallback checked only `exp` and `iss` read out of
        # the unverified payload.
        #
        # A timeout is an availability failure. Converting it into an
        # authentication decision is the bug: the one thing an auth control may
        # never do is become more permissive when a dependency is unreachable.
        #
        # Offline sign-in is a real capability and this removes it. Restoring it
        # correctly means verifying the signature against CACHED Google JWKS
        # (CIRISAgent#999) — offline, but still verified. Not by trusting the
        # token.
        logger.warning("[NativeAuth] Google tokeninfo API timed out — refusing (no unverified fallback)")
        raise HTTPException(
...
def _oauth_config_file_exists() -> bool:
    """Check whether an oauth.json exists at either of the documented paths."""
    if OAUTH_CONFIG_PATH.exists():
        return True
    return (Path.home() / OAUTH_CONFIG_DIR / OAUTH_CONFIG_FILE).exists()


def _get_apple_platform_default_audiences() -> Optional[Set[str]]:
    """Return default Apple audiences for iOS/macOS when no oauth.json exists.
    ...
    Returns None on non-Apple platforms (oauth.json is required there).
    """
    import sys
...
def _get_allowed_apple_audiences_from_config() -> Set[str]:
    ...
    try:
        ...
        if exc.status_code == status.HTTP_404_NOT_FOUND:
            # _load_oauth_config raises 404 in two cases — file missing, OR
            # file present but lacking the "apple" provider. Only the first
            # case should fall back to platform-default audiences. If the
            # operator deployed an oauth.json without apple, that is an
            # explicit decision to disable apple sign-in and we MUST honor
            # it (returning bundle-ID defaults would silently re-enable a
            # provider the operator turned off).
            if not _oauth_config_file_exists():
