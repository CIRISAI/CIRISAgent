...
### Supported Languages
`am` (Amharic), `ar` (Arabic), `bn` (Bengali), `de` (German), `en` (English), `es` (Spanish), `fr` (French), `hi` (Hindi), `it` (Italian), `ja` (Japanese), `ko` (Korean), `pt` (Portuguese), `ru` (Russian), `sw` (Swahili), `tr` (Turkish), `ur` (Urdu), `zh` (Chinese)
