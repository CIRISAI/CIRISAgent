...
class AdapterConfigurationService:
    ...
    async def _execute_step_type(
        ...
        if step.step_type == "discovery":
            ...
            if "code" in step_data:
                ...
                # Pass callback_base_url if it was stored during OAuth URL generation
    ...
    async def restore_persisted_adapters(self, config_service: Any) -> int:
        ...
        for adapter_type, config in persisted_configs.items():
            ...
            try:
                ...
                if success:
                    restored_count += 1
                    logger.info(f"Restored adapter from persisted config: {adapter_type}")
                else:
