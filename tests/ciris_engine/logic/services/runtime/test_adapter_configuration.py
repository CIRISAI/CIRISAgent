...
class TestAdapterConfigurationPersistence:
    """Tests for adapter configuration persistence (load-on-startup)."""
    ...
    async def test_persist_adapter_config_success(self) -> None:
        ...
        assert call_args[0][1]["load_on_startup"] is True

    @pytest.mark.asyncio
    async def test_persist_adapter_config_no_service(self) -> None:
        """Test persist returns False when no config service available."""
        success = await self.service.persist_adapter_config(
            adapter_type="homeassistant",
            config={"key": "value"},
            config_service=None,
    ...
    async def test_load_persisted_configs(self) -> None:
        ...
        mock_config_service.get_all_with_prefix = AsyncMock(
            return_value={
                "adapter.startup.homeassistant": {
                    "adapter_type": "homeassistant",
                    "config": {"base_url": "https://ha.local:8123"},
                    "load_on_startup": True,
                },
                "adapter.startup.nest": {
                    "adapter_type": "nest",
        ...
        configs = await self.service.load_persisted_configs(mock_config_service)

        assert len(configs) == 2
        assert "homeassistant" in configs
        assert "nest" in configs
        assert configs["homeassistant"]["base_url"] == "https://ha.local:8123"

    @pytest.mark.asyncio
    async def test_load_persisted_configs_no_service(self) -> None:
        """Test load returns empty dict when no config service available."""
        configs = await self.service.load_persisted_configs(None)
        assert configs == {}
    ...
    async def test_load_persisted_configs_filters_disabled(self) -> None:
        ...
        assert "disabled" not in configs

    @pytest.mark.asyncio
    async def test_restore_persisted_adapters(self) -> None:
        """Test restoring adapters from persisted configurations."""
        self.service.register_adapter_config(
            adapter_type="homeassistant",
            interactive_config=self.test_config,
            adapter_instance=self.mock_adapter,
        ...
        restored_count = await self.service.restore_persisted_adapters(mock_config_service)

        assert restored_count == 1

    @pytest.mark.asyncio
    async def test_restore_persisted_adapters_validation_fails(self) -> None:
        """Test restore skips adapters with invalid configuration."""
        self.mock_adapter.validation_result = (False, "Invalid config")
        self.service.register_adapter_config(
            adapter_type="homeassistant",
            interactive_config=self.test_config,
            adapter_instance=self.mock_adapter,
    ...
    async def test_remove_persisted_config(self) -> None:
        ...
        mock_config_service = AsyncMock()
        mock_config_service.delete = AsyncMock()

        success = await self.service.remove_persisted_config(
            adapter_type="homeassistant",
            config_service=mock_config_service,
        )

        assert success is True
        mock_config_service.delete.assert_called_once_with("adapter.startup.homeassistant")

    @pytest.mark.asyncio
    async def test_remove_persisted_config_no_service(self) -> None:
        """Test remove returns False when no config service available."""
        success = await self.service.remove_persisted_config(
            adapter_type="homeassistant",
            config_service=None,
        )
    ...
    @pytest.mark.asyncio
    async def test_remove_persisted_config_exception(self) -> None:
        """Test remove handles exceptions gracefully."""
        mock_config_service = AsyncMock()
        mock_config_service.delete = AsyncMock(side_effect=Exception("Database error"))
