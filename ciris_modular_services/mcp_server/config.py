...
class MCPServerSecurityConfig(BaseModel):
    ...
    allowed_clients: List[str] = Field(
        default_factory=list, description="Allowed client identifiers (empty = all)"
    )
    blocked_clients: List[str] = Field(
        default_factory=list, description="Blocked client identifiers"
    )
...
class MCPServerExposureConfig(BaseModel):
    ...
    tool_allowlist: List[str] = Field(
        default_factory=list, description="Tools to expose (empty = all allowed)"
    )
    tool_blocklist: List[str] = Field(
        default_factory=list, description="Tools to never expose"
    )

    # Resource exposure
    expose_resources: bool = Field(True, description="Expose CIRIS resources")
    resource_allowlist: List[str] = Field(
        default_factory=list, description="Resource URIs to expose"
    )
    resource_blocklist: List[str] = Field(
        default_factory=list, description="Resource URIs to never expose"
    )

    # Prompt exposure
    expose_prompts: bool = Field(True, description="Expose CIRIS prompts")
    prompt_allowlist: List[str] = Field(
        default_factory=list, description="Prompts to expose"
    )
    prompt_blocklist: List[str] = Field(
        default_factory=list, description="Prompts to never expose"
    )
...
class MCPServerAdapterConfig(BaseModel):
    ...
    config_key_prefix: str = Field(
        "mcp_server", description="Config key prefix for graph storage"
    )
