...
- **No first-class `autonomy:human_centric_design` event** — the design discipline is policy-level and code-level (defaults to TEMPORARY consent, AIR triggers, opt-in required for PARTNERED) but no module emits a typed `autonomy:human_centric_design` attestation per session.
- **No `autonomy:agent_self_determination` introspection event** — MH's wire form (autonomy of the agent itself) is conceptually present in the agent's deferral capability ("I am not the right party to decide this") but there is no structured event that records *the agent's own autonomy* having been preserved against, e.g., an originator override attempt.
