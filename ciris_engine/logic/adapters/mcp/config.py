...
class MCPSecurityConfig(BaseModel):
    ...
    allowed_tools: List[str] = Field(
        default_factory=list, description="Whitelist of allowed tool names (empty = all)"
    )
    blocked_tools: List[str] = Field(
        default_factory=list, description="Blacklist of blocked tool names"
    )
    ...
    detect_tool_poisoning: bool = Field(
        True, description="Detect malicious instructions in tool descriptions"
    )
    ...
    require_resource_indicator: bool = Field(
        True, description="Require resource indicator in token requests"
    )
    token_audience_validation: bool = Field(
        True, description="Validate token audience matches server"
    )
...
class MCPServerConfig(BaseModel):
    ...
    transport: MCPTransportType = Field(
        MCPTransportType.STDIO, description="Transport type for server connection"
    )
    ...
    bus_bindings: List[MCPBusBinding] = Field(
        default_factory=list, description="Bus bindings for this server"
    )

    # Security
    security: MCPSecurityConfig = Field(
        default_factory=MCPSecurityConfig, description="Security configuration"
    )
...
class MCPAdapterConfig(BaseModel):
    ...
    config_key_prefix: str = Field(
        "mcp.servers", description="Config key prefix for graph storage"
    )

    # Adapter settings
    adapter_id: str = Field("mcp_default", description="Adapter identifier")
    auto_discover_servers: bool = Field(
        False, description="Auto-discover MCP servers from environment"
    )
