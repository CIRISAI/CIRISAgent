...
class Adapter(Service):
    ...
    def _initialize_config(self, kwargs: MCPAdapterKwargs) -> None:
        ...
        logger.info(
            f"MCP Adapter configured with {len(self.config.servers)} server(s), "
    ...
    async def _connect_mcp_server(self, server_config: MCPServerConfig) -> Optional[Any]:
        ...
        Returns:
            MCP client instance or None on failure
        """
    ...
    async def _connect_stdio_server(self, server_config: MCPServerConfig) -> Optional[Any]:
        ...
        try:
            ...
            try:
                ...
                return self._create_mock_client(server_config)
            ...
            async with stdio_client(server_params) as (read, write):
                async with ClientSession(read, write) as session:
                    ...
                    logger.info(
                        f"Connected to MCP server '{server_config.server_id}' via stdio"
                    )
            ...
            logger.error(
                f"Failed to connect to stdio MCP server '{server_config.server_id}': {e}"
            )
            return None

    async def _connect_sse_server(self, server_config: MCPServerConfig) -> Optional[Any]:
    ...
    async def _connect_streamable_http_server(self, server_config: MCPServerConfig) -> Optional[Any]:
        """Connect to a Streamable HTTP MCP server."""
        # Similar to SSE, but uses streamable HTTP transport
        return await self._connect_sse_server(server_config)

    async def _connect_websocket_server(self, server_config: MCPServerConfig) -> Optional[Any]:
            ...
            async def list_tools(self) -> Any:
                ...
                class MockTools:
                    tools = [
                        {
                            "name": "mock_tool",
                            "description": f"Mock tool from {config.name}",
                            "inputSchema": {
                                "type": "object",
    ...
    def get_services_to_register(self) -> List[AdapterServiceRegistration]:
        ...
        has_tool_binding = any(
            any(b.bus_type == MCPBusType.TOOL for b in s.bus_bindings)
            for s in self.config.servers
            if s.enabled
        )

        has_wise_binding = any(
            any(b.bus_type == MCPBusType.WISE for b in s.bus_bindings)
            for s in self.config.servers
            if s.enabled
        )
    ...
    async def start(self) -> None:
        ...
        for server_config in self.config.servers:
            ...
            # Connect to server
            client = await self._connect_mcp_server(server_config)
            if not client:
    ...
    async def update_server_bus_bindings(
        self, server_id: str, bus_bindings: List[Dict[str, Any]]
    ) -> bool:
