...
class MCPToolService:
        ...
        try:
            ...
            if hasattr(client, "list_tools"):
                ...
                for tool in tools:
                    ...
                    allowed, violation = await self._security_manager.check_tool_access(
                ...
                logger.debug(
                    f"Refreshed tools cache for '{server_id}': "
                    f"{len(self._tools_cache[server_id])} tools available"
                )
    ...
    async def execute_tool(
        ...
        try:
            # Validate input
            allowed, violation = await self._security_manager.validate_input(
                server_id, original_name, parameters
            )
            if not allowed:
                self._blocked += 1
            ...
            if hasattr(client, "call_tool"):
                ...
                # Validate output
                allowed, violation = await self._security_manager.validate_output(
                    server_id, original_name, data
                )
                if not allowed:
                    self._blocked += 1
                    return ToolExecutionResult(
