...
TOOL_DEFINITIONS: Dict[str, ToolInfo] = {
    "your_tool": ToolInfo(
        ...
        dma_guidance=ToolDMAGuidance(
            requires_approval=False,  # advisory only — does NOT gate execution (#942)
            min_confidence=0.8,
...
> ### ⚠️ Nothing on `ToolDMAGuidance` is enforced
>
> Read this before the field docs below. `ToolDMAGuidance` is **advisory prompt
> content**. It shapes what the model is told; it does not constrain what the
> runtime will do. There is no interception between action selection and
> dispatch that consults any field on it. Tracked as
> [#942](https://github.com/CIRISAI/CIRISAgent/issues/942); the full picture is
> in `FSD/THREAT_MODEL_2.9.7.md`.
...
**What `True` actually does — the complete list:**

1. `ciris_engine/logic/dma/tsaspdma.py:247` appends the line
   `**⚠️ Requires wise authority approval**` to the action-selection prompt.
2. `ciris_engine/logic/services/tool/tool_disclosure.py:118` adds a
   `REQUIRES_APPROVAL` capability label to the first-run consent disclosure the
   operator sees in the setup wizard.

**What it does not do:** it does not trigger the Wise Authority deferral
workflow, does not create an approval record, does not interpose a human, and
does not block dispatch. No handler, bus, or conscience reads it. A model that
selects a `requires_approval=True` tool executes that tool immediately.

Two shipped tools set it — `shell_command`
(`ciris_engine/logic/adapters/cli/cli_tools.py:492`) and `send_money`
(`ciris_adapters/wallet/tool_service.py:183`) — and it has never gated either.
Skills generated by the skill-import wizard default it to `True`
(`logic/services/skill_import/builder.py:348`), into the same dead field.

Set it anyway: it is honest metadata and it is what #942 will make load-bearing.
But **design your adapter as though the tool will run the instant the model
picks it**, because that is what happens today. If your tool needs a real gate,
it needs a deterministic one on its own execution path — the pattern to copy is
the budget envelope in `FSD/BUDGET_ENVELOPE.md`, which sits in
`_execute_send_money` and denies by default.

### `min_confidence`

Intended as the minimum confidence score (0.0-1.0) for the ASPDMA to select this
tool. **It is currently read by nothing** — not the DMA, not the handler, and
not even the prompt renderer (`_format_dma_guidance` renders
`when_not_to_use`, `ethical_considerations`, `prerequisite_actions` and
`requires_approval`, and skips `min_confidence` and `followup_actions`). Its only
non-test reference is `logic/services/skill_import/builder.py:534`, which
codegens the literal back into generated adapter source. Also #942.

The conventional values below describe the intent, not an enforced behaviour:
...
- [ ] `requires_approval=True` for destructive/financial tools (metadata only — it
      gates nothing, #942; if the tool needs a real gate, add a deterministic one
      on its own execution path)
