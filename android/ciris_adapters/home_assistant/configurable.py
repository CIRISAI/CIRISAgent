...
class HAConfigurableAdapter:
    ...
    async def get_oauth_url(
        ...
        if redirect_uri and redirect_uri.startswith("ciris://"):
            # Android deep link - use ciris:// scheme
            # For HA IndieAuth, we still need a valid client_id
            # Use the callback_base_url as client_id if available, else production
            client_id = callback_base_url.rstrip("/") if callback_base_url else self._client_id
            final_redirect_uri = redirect_uri
            logger.info("=" * 60)
            logger.info("[OAUTH] ANDROID DEEP LINK REDIRECT URI:")
            logger.info(f"  platform: {platform}")
            logger.info(f"  redirect_uri: {final_redirect_uri}")
            logger.info(f"  client_id: {client_id}")
            logger.info(f"  state/session_id: {state}")
            logger.info(f"  Deep link will be handled by OAuthCallbackActivity")
            logger.info("=" * 60)
        elif callback_base_url:
            ...
            final_redirect_uri = f"{self._client_id}/v1/auth/oauth/home_assistant/callback"
            client_id = self._client_id
            logger.info(f"[OAUTH] Using production callback: {final_redirect_uri}")
    ...
    async def handle_oauth_callback(
        ...
        # Use callback_base_url as client_id if provided (for local OAuth)
        # This must match the client_id used in get_oauth_url
        client_id = callback_base_url.rstrip("/") if callback_base_url else self._client_id
