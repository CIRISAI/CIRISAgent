...
from ciris_engine.logic.adapters.api.routes.users import ChangePasswordRequest, change_password, list_user_api_keys
...
class TestPasswordChangeDependencyFix:
    ...
    def mock_auth_service(self):
        ...
        service.get_user = Mock(
            return_value=Mock(wa_id="admin", name="Admin User", api_role=APIRole.SYSTEM_ADMIN, custom_permissions=None)
        )
    ...
    async def test_change_password_admin_changing_other_user(self, auth_context_admin, mock_auth_service):
        ...
        request = ChangePasswordRequest(current_password="old_password", new_password="new_secure_password")

        # This should NOT raise AttributeError anymore
        result = await change_password(
            user_id="other_user", request=request, auth=auth_context_admin, auth_service=mock_auth_service
        )

        # Verify the service was called correctly
        mock_auth_service.change_password.assert_called_once_with(
            user_id="other_user", new_password="new_secure_password", skip_current_check=True
        )
    ...
    async def test_change_password_user_changing_own(self, auth_context_user, mock_auth_service):
        ...
        request = ChangePasswordRequest(current_password="current_password", new_password="new_secure_password")
        ...
        mock_auth_service.change_password.assert_called_once_with(
            user_id="user123", new_password="new_secure_password", current_password="current_password"
        )
        assert "message" in result
    ...
    async def test_permission_check_insufficient_permissions(self, auth_context_user, mock_auth_service):
        ...
        mock_auth_service.get_user = Mock(
            return_value=Mock(wa_id="user123", name="Test User", api_role=APIRole.OBSERVER, custom_permissions=None)
        )
