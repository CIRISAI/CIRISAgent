...
- [x] [**CIRISPersist#66** — detected-secret store + process_incoming_text wiring.](https://github.com/CIRISAI/CIRISPersist/issues/66) `secrets_process_incoming_text` is a stub (always returns empty `refs`). `secrets_store_secret` is keyed-by-name and doesn't accept the agent's detected-secret metadata (pattern, sensitivity, context_hint, auto_decapsulate rules, source_message_id) with an agent-assigned UUID. Blocks Phase 2a. **Landed in persist 1.5.24; Phase 2a shipped in CIRISAgent e6f9ed953.**
- [ ] [**CIRISPersist#67** — cirisgraph_query_nodes attribute-path filter.](https://github.com/CIRISAI/CIRISPersist/issues/67) Surfaced during Phase 4 migration of `memory_query_helpers.py`. OBSERVER user filter requires JSON-path equality against `attributes.created_by` and `attributes.user_list`. Persist 1.6.0's NodeFilter doesn't expose any attribute-path predicate. Blocks Phase 5 bootstrap teardown — `memory_query_helpers` keeps raw SQL until this lands.
- [ ] [**CIRISPersist#68** — TSDB consolidation: non-metric summary types.](https://github.com/CIRISAI/CIRISPersist/issues/68) `telemetry_consolidate_period` (v1.6.0) only handles METRIC summaries. The agent's TSDB pipeline produces 5 summary types (metric, task, conversation, trace, audit); the other 4 source from tables persist doesn't yet aggregate. Blocks Phase 3b completion + Phase 5 bootstrap teardown.
