...
from ciris_engine.logic.adapters.api.routes.system.helpers import (
    ...
    map_service_type_enum,
    parse_direct_service_key,
    parse_registry_service_key,
    parse_service_key,
...
class TestRuntimeControlHelpers:
    ...
    def test_get_runtime_control_service_main_service(self):
        """Test get_runtime_control_service with main service available."""
        ...
        result = get_runtime_control_service(request)

        assert result == main_service

    def test_get_runtime_control_service_fallback_service(self):
        """Test get_runtime_control_service with fallback service."""
        request = Mock(spec=Request)
    ...
    def test_get_runtime_control_service_no_service(self):
        """Test get_runtime_control_service with no service available."""
        request = Mock(spec=Request)
        request.app.state.main_runtime_control_service = None
        request.app.state.runtime_control_service = None

        with pytest.raises(HTTPException) as exc_info:
            get_runtime_control_service(request)

        assert exc_info.value.status_code == 503

    def test_validate_runtime_action_valid(self):
        """Test validate_runtime_action with valid actions."""
        for action in ["pause", "resume", "state"]:
            validate_runtime_action(action)  # Should not raise

    def test_validate_runtime_action_invalid(self):
        """Test validate_runtime_action with invalid action."""
        with pytest.raises(HTTPException) as exc_info:
            validate_runtime_action("invalid")
    ...
    async def test_execute_pause_action_api_service(self):
        ...
        with patch("ciris_engine.logic.adapters.api.routes.system.helpers.inspect.signature") as mock_signature:
            ...
            result = await execute_pause_action(runtime_control, "Test reason")
    ...
    def test_extract_pipeline_state_info_no_runtime(self):
        """Test extract_pipeline_state_info with no runtime available."""
        request = Mock(spec=Request)
    ...
    def test_extract_pipeline_state_info_no_pipeline_controller(self):
        """Test extract_pipeline_state_info with no pipeline controller."""
        request = Mock(spec=Request)
        runtime = Mock()
    ...
    def test_extract_pipeline_state_info_with_current_step(self):
        """Test extract_pipeline_state_info with current step available."""
        request = Mock(spec=Request)
        runtime = Mock()
        agent_processor = Mock()
    ...
    def test_extract_pipeline_state_info_controller_error(self):
        """Test extract_pipeline_state_info with controller error."""
        request = Mock(spec=Request)
        runtime = Mock()
        agent_processor = Mock()
        pipeline_controller = Mock()
    ...
    def test_create_pause_response_success(self):
        """Test create_pause_response with successful pause."""
        result = create_pause_response(True, "BUILD_CONTEXT", {"step_point": "BUILD_CONTEXT"}, {"state": "paused"})
    ...
    def test_create_pause_response_already_paused(self):
        """Test create_pause_response when already paused."""
        result = create_pause_response(False, None, None, None)
    ...
    async def test_execute_resume_action_main_service(self):
        """Test execute_resume_action with main service response object."""
        runtime_control = Mock()
        resume_result = Mock()
    ...
    async def test_execute_resume_action_api_service(self):
        """Test execute_resume_action with API service boolean response."""
        runtime_control = Mock()
    ...
    async def test_execute_resume_action_not_paused(self):
        """Test execute_resume_action when not paused."""
        runtime_control = Mock()
    ...
    async def test_execute_state_action_no_queue(self):
        """Test execute_state_action with no queue status."""
        runtime_control = Mock()
        status = Mock()
    ...
    def test_get_cognitive_state_no_runtime(self):
        """Test get_cognitive_state with no runtime."""
        request = Mock(spec=Request)
        request.app.state.runtime = None

        result = get_cognitive_state(request)

        assert result is None

    def test_get_cognitive_state_no_agent_processor(self):
        """Test get_cognitive_state with no agent processor."""
        request = Mock(spec=Request)
        runtime = Mock()
    ...
    def test_get_cognitive_state_success(self):
        """Test get_cognitive_state with successful retrieval."""
        request = Mock(spec=Request)
        runtime = Mock()
        agent_processor = Mock()
    ...
    def test_get_cognitive_state_error(self):
        """Test get_cognitive_state with error during retrieval."""
        request = Mock(spec=Request)
        runtime = Mock()
        agent_processor = Mock()
    ...
    def test_create_final_response_basic(self):
        """Test create_final_response with basic response."""
        ...
        result = create_final_response(base_result, "WORK")
    ...
    def test_create_final_response_with_enhanced_fields(self):
        """Test create_final_response with enhanced fields."""
        base_result = RuntimeControlResponse(
    ...
    def test_create_final_response_cognitive_state_fallback(self):
        """Test create_final_response with cognitive state fallback."""
        base_result = RuntimeControlResponse(
        ...
        result = create_final_response(base_result, None)
...
class TestServiceParsingHelpers:
    ...
    def test_parse_direct_service_key_valid(self):
        """Test parse_direct_service_key with valid key."""
        service_type, display_name = parse_direct_service_key("direct.graph.memory_service")

        assert service_type == "graph"
        assert display_name == "MemoryService"

    def test_parse_direct_service_key_complex_name(self):
        """Test parse_direct_service_key with complex service name."""
        service_type, display_name = parse_direct_service_key("direct.infrastructure.time_service_provider")

        assert service_type == "infrastructure"
        assert display_name == "TimeServiceProvider"

    def test_parse_direct_service_key_invalid(self):
        """Test parse_direct_service_key with invalid key."""
        service_type, display_name = parse_direct_service_key("invalid.key")

        assert service_type == "unknown"
        assert display_name == "invalid.key"

    def test_parse_registry_service_key_4_part(self):
        """Test parse_registry_service_key with 4-part key."""
        service_type, display_name = parse_registry_service_key("registry.ServiceType.TOOL.APIToolService_12345")

        assert service_type == "tool"
        assert display_name == "API-TOOL"

    def test_parse_registry_service_key_3_part(self):
        """Test parse_registry_service_key with 3-part key."""
        service_type, display_name = parse_registry_service_key("registry.ServiceType.COMMUNICATION")
    ...
    def test_parse_registry_service_key_discord(self):
        """Test parse_registry_service_key with Discord service."""
        service_type, display_name = parse_registry_service_key(
    ...
    def test_map_service_type_enum_communication(self):
        """Test map_service_type_enum with COMMUNICATION type."""
        service_type, display_name = map_service_type_enum("ServiceType.COMMUNICATION", "TestService", "API")

        assert service_type == "adapter"
        assert display_name == "API-COMM"

    def test_map_service_type_enum_memory(self):
        """Test map_service_type_enum with MEMORY type."""
        service_type, display_name = map_service_type_enum("ServiceType.MEMORY", "MemoryService", "")

        assert service_type == "graph"
        assert display_name == "MemoryService"

    def test_map_service_type_enum_llm(self):
        """Test map_service_type_enum with LLM type."""
        service_type, display_name = map_service_type_enum("ServiceType.LLM", "LLMService", "")

        assert service_type == "runtime"
        assert display_name == "LLMService"

    def test_map_service_type_enum_time(self):
        """Test map_service_type_enum with TIME type."""
        service_type, display_name = map_service_type_enum("ServiceType.TIME", "TimeService", "")

        assert service_type == "infrastructure"
        assert display_name == "TimeService"

    def test_map_service_type_enum_tool(self):
        """Test map_service_type_enum with TOOL type."""
        service_type, display_name = map_service_type_enum("ServiceType.TOOL", "ToolService", "CLI")

        assert service_type == "tool"
        assert display_name == "CLI-TOOL"

    def test_map_service_type_enum_wise_authority(self):
        """Test map_service_type_enum with WISE_AUTHORITY type."""
        service_type, display_name = map_service_type_enum("ServiceType.WISE_AUTHORITY", "WiseService", "DISCORD")

        assert service_type == "governance"
        assert display_name == "DISCORD-WISE"

    def test_map_service_type_enum_runtime_control(self):
        """Test map_service_type_enum with RUNTIME_CONTROL type."""
        service_type, display_name = map_service_type_enum("ServiceType.RUNTIME_CONTROL", "RuntimeService", "API")

        assert service_type == "runtime"
        assert display_name == "API-RUNTIME"

    def test_map_service_type_enum_unknown(self):
        """Test map_service_type_enum with unknown type."""
        service_type, display_name = map_service_type_enum("ServiceType.UNKNOWN", "UnknownService", "")

        assert service_type == "unknown"
        assert display_name == "UnknownService"

    def test_parse_service_key_direct(self):
        """Test parse_service_key with direct service."""
        service_type, display_name = parse_service_key("direct.graph.memory_service")

        assert service_type == "graph"
        assert display_name == "MemoryService"

    def test_parse_service_key_registry(self):
        """Test parse_service_key with registry service."""
        service_type, display_name = parse_service_key("registry.ServiceType.TOOL.APIToolService_12345")

        assert service_type == "tool"
        assert display_name == "API-TOOL"

    def test_parse_service_key_unknown(self):
        """Test parse_service_key with unknown format."""
        service_type, display_name = parse_service_key("unknown.format")

        assert service_type == "unknown"
        assert display_name == "unknown.format"

    def test_create_service_status(self):
        """Test create_service_status."""
        details = {"healthy": True, "extra": "ignored"}

        status = create_service_status("direct.graph.memory_service", details)
    ...
    def test_create_service_status_unhealthy(self):
        """Test create_service_status with unhealthy service."""
        details = {"healthy": False}

        status = create_service_status("registry.ServiceType.COMMUNICATION.DiscordCommunicationService_123", details)
    ...
    def test_create_service_status_missing_healthy(self):
        """Test create_service_status with missing healthy field."""
        details = {}

        status = create_service_status("direct.infrastructure.time_service", details)
    ...
    def test_update_service_summary_new_type(self):
        """Test update_service_summary with new service type."""
        service_summary = {}

        update_service_summary(service_summary, "graph", True)

        assert service_summary == {"graph": {"total": 1, "healthy": 1}}

    def test_update_service_summary_existing_type(self):
        """Test update_service_summary with existing service type."""
        service_summary = {"graph": {"total": 2, "healthy": 1}}

        update_service_summary(service_summary, "graph", False)

        assert service_summary == {"graph": {"total": 3, "healthy": 1}}

    def test_update_service_summary_healthy_increment(self):
        """Test update_service_summary with healthy service increment."""
        service_summary = {"runtime": {"total": 0, "healthy": 0}}

        update_service_summary(service_summary, "runtime", True)
...
class TestServiceCategoryHelpers:
    ...
    def test_get_service_category_communication(self):
        """Test get_service_category with COMMUNICATION service type."""
        result = get_service_category("ServiceType.COMMUNICATION")
        assert result == "adapter"

    def test_get_service_category_memory(self):
        """Test get_service_category with MEMORY service type."""
        result = get_service_category("ServiceType.MEMORY")
        assert result == "graph"

    def test_get_service_category_llm(self):
        """Test get_service_category with LLM service type."""
        result = get_service_category("ServiceType.LLM")
        assert result == "runtime"

    def test_get_service_category_time(self):
        """Test get_service_category with TIME service type."""
        result = get_service_category("ServiceType.TIME")
        assert result == "infrastructure"

    def test_get_service_category_wise_authority(self):
        """Test get_service_category with WISE_AUTHORITY service type."""
        result = get_service_category("ServiceType.WISE_AUTHORITY")
        assert result == "governance"

    def test_get_service_category_runtime_control(self):
        """Test get_service_category with RUNTIME_CONTROL service type."""
        result = get_service_category("ServiceType.RUNTIME_CONTROL")
        assert result == "runtime"

    def test_get_service_category_tool(self):
        """Test get_service_category with TOOL service type."""
        result = get_service_category("ServiceType.TOOL")
        assert result == "tool"

    def test_get_service_category_secrets_tool(self):
        """Test get_service_category with SECRETS_TOOL service type."""
        result = get_service_category("ServiceType.SECRETS_TOOL")
        assert result == "tool"

    def test_get_service_category_infrastructure_services(self):
        """Test get_service_category with all infrastructure services."""
        services = [
            "ServiceType.SECRETS",
        ...
        for service_type in services:
            result = get_service_category(service_type)
    ...
    def test_get_service_category_graph_services(self):
        """Test get_service_category with all graph services."""
        services = [
            "ServiceType.CONFIG",
    ...
    def test_get_service_category_governance_services(self):
        """Test get_service_category with all governance services."""
    ...
    def test_get_service_category_runtime_services(self):
        """Test get_service_category with all runtime services."""
    ...
    def test_get_service_category_unknown(self):
        """Test get_service_category with unknown service type."""
        result = get_service_category("ServiceType.UNKNOWN")
        assert result == "unknown"

    def test_get_service_category_malformed(self):
        """Test get_service_category with malformed service type."""
        result = get_service_category("NotAServiceType")
        assert result == "unknown"

    def test_create_display_name_communication_with_adapter(self):
        """Test create_display_name with COMMUNICATION and adapter prefix."""
        result = create_display_name("ServiceType.COMMUNICATION", "TestService", "API")
        assert result == "API-COMM"

    def test_create_display_name_communication_with_discord(self):
        """Test create_display_name with COMMUNICATION and Discord prefix."""
        result = create_display_name("ServiceType.COMMUNICATION", "DiscordService", "DISCORD")
        assert result == "DISCORD-COMM"

    def test_create_display_name_communication_no_adapter(self):
        """Test create_display_name with COMMUNICATION but no adapter prefix."""
        result = create_display_name("ServiceType.COMMUNICATION", "CommunicationService", "")
        assert result == "CommunicationService"

    def test_create_display_name_tool_with_adapter(self):
        """Test create_display_name with TOOL and adapter prefix."""
        result = create_display_name("ServiceType.TOOL", "ToolService", "CLI")
        assert result == "CLI-TOOL"

    def test_create_display_name_tool_no_adapter(self):
        """Test create_display_name with TOOL but no adapter prefix."""
        result = create_display_name("ServiceType.TOOL", "ToolService", "")
        assert result == "ToolService"

    def test_create_display_name_wise_authority_with_adapter(self):
        """Test create_display_name with WISE_AUTHORITY and adapter prefix."""
        result = create_display_name("ServiceType.WISE_AUTHORITY", "WiseService", "DISCORD")
        assert result == "DISCORD-WISE"

    def test_create_display_name_wise_authority_no_adapter(self):
        """Test create_display_name with WISE_AUTHORITY but no adapter prefix."""
        result = create_display_name("ServiceType.WISE_AUTHORITY", "WiseService", "")
        assert result == "WiseService"

    def test_create_display_name_runtime_control_with_adapter(self):
        """Test create_display_name with RUNTIME_CONTROL and adapter prefix."""
        result = create_display_name("ServiceType.RUNTIME_CONTROL", "RuntimeService", "API")
        assert result == "API-RUNTIME"

    def test_create_display_name_runtime_control_no_adapter(self):
        """Test create_display_name with RUNTIME_CONTROL but no adapter prefix."""
        result = create_display_name("ServiceType.RUNTIME_CONTROL", "RuntimeService", "")
        assert result == "RuntimeService"

    def test_create_display_name_other_service_with_adapter(self):
        """Test create_display_name with other service types ignores adapter prefix."""
        result = create_display_name("ServiceType.MEMORY", "MemoryService", "IGNORED")
        assert result == "MemoryService"

    def test_create_display_name_other_service_no_adapter(self):
        """Test create_display_name with other service types and no adapter prefix."""
        result = create_display_name("ServiceType.LLM", "LLMService", "")
        assert result == "LLMService"

    def test_create_display_name_unknown_service(self):
        """Test create_display_name with unknown service type."""
        result = create_display_name("ServiceType.UNKNOWN", "UnknownService", "PREFIX")
