...
class TestConsentServiceCleanup:
    ...
    @pytest.fixture
    def enhanced_consent_service(
        self, mock_time_service, mock_memory_bus, conversation_summary_nodes, task_summary_nodes
    ):
    ...
    def test_find_expired_from_cache_mixed_consents(
        self, consent_service, mock_time_service, expired_consent_status, valid_consent_status, permanent_consent_status
    ):
    ...
    async def test_full_cleanup_integration_cache_fallback(
        ...
        service._consent_cache = {"expired_user": expired_consent_status, "valid_user": valid_consent_status}
    ...
    async def test_get_impact_report_with_real_tsdb_data(self, enhanced_consent_service, mock_time_service):
        ...
        assert (
            result.users_helped == 5
        )  # All unique participants from ALL conversations excluding test_user (other_user_1, other_user_2, another_user, random_user, other_random)
