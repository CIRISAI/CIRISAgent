...
async def auth_service(temp_auth_db, time_service, temp_key_dir):
    ...
    service = AuthenticationService(db_path=temp_auth_db, time_service=time_service, key_dir=temp_key_dir)
    await service.start()
    yield service
    await service.stop()
...
async def test_disk_persistence_verification(auth_service, temp_auth_db):
    ...
    new_auth_service = AuthenticationService(
        db_path=temp_auth_db, time_service=new_time_service, key_dir=auth_service.key_dir
    )
...
async def test_password_authentication_works(auth_service):
    ...
    assert (
        auth_service._verify_password(correct_password, hashed_password) is True
    ), "Correct password should verify successfully"

    # Test wrong password verification
    assert (
        auth_service._verify_password(wrong_password, hashed_password) is False
    ), "Wrong password should fail verification"

    # Test edge cases
    assert auth_service._verify_password("", hashed_password) is False, "Empty password should fail"
    assert auth_service._verify_password(correct_password, "") is False, "Empty hash should fail"
    assert auth_service._verify_password(correct_password, "invalid_hash") is False, "Invalid hash format should fail"
...
async def test_default_admin_password_protection(auth_service):
    ...
    try:
        ...
        assert (
            auth_service._verify_password(default_password, original_admin.password_hash) is True
