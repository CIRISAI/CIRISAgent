...
class RuntimeBootstrapConfig(BaseModel):
    ...
    adapters: List[AdapterLoadRequest] = Field(
        default_factory=list, description="List of adapters to load at startup"
    )
    adapter_overrides: Dict[str, AdapterConfig] = Field(
        default_factory=dict, description="Adapter-specific configuration overrides by adapter type"
    )
    ...
    preload_tasks: List[str] = Field(
        default_factory=list, description="Tasks to preload after WORK state transition"
    )
