...
- **`goal:planet` is not yet a first-class scale value.** The corpus (T3-06 in seed `aggregate_indices.t3_candidates_v1_5`) reinforces the planet scale (MH + IEEE Ch4 + IEEE Ch8) but `GraphScope` ends at COMMUNITY. Planetary-scale belonging (ecological, biospheric) currently composes through species + ENVIRONMENT scope rather than being a named cohort. Closure depends on either extending `GraphScope` (agent-side) or on the federation wire format admitting `goal:planet` as a {scale} value the agent maps onto an existing scope at trace-emit time. T3-09 `beneficence:planetary_biosystem_intrinsic_value` (per seed) is the linked beneficence-side reinforcement.
