...
class TestDiscordAdapter:
    ...
    def setup_test_db(self) -> Generator[str, None, None]:
        ...
        `initialize_database` runs all migrations and wires the persist
        Engine into `persistence.models.graph` — all migrated callers
        (`tasks`, `thoughts`, `correlations`, …) route through that single
        engine, so the legacy `get_db_connection` mock dance is no longer
        needed.
        """
