...
class TestSingleStepFixValidation:
    ...
    @pytest.fixture
    def fixed_agent_processor(
        self,
        mock_config,
        mock_services,
        mock_non_hanging_state_processor,
        mock_fixed_pipeline_controller,
        mock_time_service,
    ):
            ...
            try:
                ...
                with patch("ciris_engine.logic.persistence.get_thoughts_by_status") as mock_get_thoughts:
                    ...
                    if hasattr(fixed_agent_processor._pipeline_controller, "execute_single_step_point"):
                        step_result = fixed_agent_processor._pipeline_controller.execute_single_step_point(
                            sample_thought
                        )
                    else:
                        # Fallback to async version
                        step_result = await fixed_agent_processor._pipeline_controller.execute_single_step_point_async(
                            sample_thought
                        )
                ...
                return {
                    "success": False,
                    "error": str(e),
                    "processing_time_ms": (fixed_agent_processor._time_service.now() - start_time).total_seconds()
                    * 1000,
                }
    ...
    async def test_single_step_fix_error_handling(self, fixed_agent_processor, sample_thought):
        ...
        fixed_agent_processor._pipeline_controller.execute_single_step_point = AsyncMock(
            side_effect=mock_error_execution
        )
