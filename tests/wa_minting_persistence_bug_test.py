...
class TestWAMintingPersistenceBug:
    ...
    def wa_auth_service(self, temp_db_path, mock_time_service):
        ...
        return AuthenticationService(db_path=temp_db_path, time_service=mock_time_service)

    @pytest.mark.asyncio
    async def test_oauth_user_creation_does_not_create_wa_certificate(self, api_auth_service, wa_auth_service):
        """
        ...
        oauth_user = api_auth_service.create_oauth_user(
            provider=provider, external_id=external_id, email=email, name=name, role=role
        )
    ...
    @pytest.mark.asyncio
    async def test_what_should_happen_oauth_with_wa_creation(self, api_auth_service, wa_auth_service):
        """
        ...
        if role == UserRole.AUTHORITY:
            ...
            await wa_auth_service.update_wa(wa_cert.wa_id, oauth_provider=provider, oauth_external_id=external_id)
    ...
    @pytest.mark.asyncio
    async def test_bug_reproduction_exact_user_scenario(self, api_auth_service, wa_auth_service):
        """
    ...
    @pytest.mark.asyncio
    async def test_fix_mint_wise_authority_creates_wa_certificate(self, api_auth_service, wa_auth_service):
        """
        ...
        minted_user = await api_auth_service.mint_wise_authority(
            user_id=user_id, wa_role=WARole.AUTHORITY, minted_by="test-minter"
        )
    ...
    async def test_mint_wa_updates_existing_certificate(self, api_auth_service, wa_auth_service):
        ...
        oauth_user = api_auth_service.create_oauth_user(
            provider=provider, external_id=external_id, email=email, name=name, role=UserRole.ADMIN
        )

        # Step 2: Create WA certificate manually first using get_wa with user_id
        # This simulates existing WA that should be updated
        wa_cert = await wa_auth_service.create_wa(name=name, email=email, scopes=["wa.read"], role=WARole.OBSERVER)
    ...
    async def test_mint_wa_handles_email_formats(self, api_auth_service, wa_auth_service):
        ...
        for name, input_email, expected_email in test_cases:
            ...
            api_auth_service.create_oauth_user(
                provider="test", external_id=name, email=input_email, name=name, role=UserRole.ADMIN
            )
    ...
    async def test_mint_wa_preserves_base_permissions(self, api_auth_service, wa_auth_service):
        ...
        # Mint as WA
        await api_auth_service.mint_wise_authority(user_id=user_id, wa_role=WARole.AUTHORITY, minted_by="test-minter")
