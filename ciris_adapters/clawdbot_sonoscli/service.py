"""
Sonoscli Tool Service for CIRIS.
...
class SonoscliToolService:
    """
    Sonoscli tool service providing skill-based guidance.
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self._call_count = 0
        logger.info("SonoscliToolService initialized")

    async def start(self) -> None:
        """Start the service."""
        logger.info("SonoscliToolService started")

    async def stop(self) -> None:
        """Stop the service."""
        logger.info("SonoscliToolService stopped")
    ...
    def _build_tool_info(self) -> ToolInfo:
        ...
        return ToolInfo(
            ...
            category="skill",
            when_to_use="""When you need to control sonos speakers (discover/status/play/volume/group).""",
            requirements=ToolRequirements(
                binaries=[
                    BinaryRequirement(name="sonos"),
                ],
            ),
            install_steps=[
                InstallStep(id="go", kind="go", label="Install sonoscli (go)", provides_binaries=["sonos"]),
            ],
            documentation=ToolDocumentation(
                quick_start="Use `sonos` to control Sonos speakers on the local network.",
                detailed_instructions="""# Sonos CLI\n\nUse `sonos` to control Sonos speakers on the local network.\n\nQuick start\n- `sonos discover`\n- `sonos status --name \"Kitchen\"`\n- `sonos play|pause|stop --name \"Kitchen\"`\n- `sonos volume set 15 --name \"Kitchen\"`\n\nCommon tasks\n- Grouping: `sonos group status|join|unjoin|party|solo`\n- Favorites: `sonos favorites list|open`\n- Queue: `sonos queue list|play|clear`\n- Spotify search (via SMAPI): `sonos smapi search --service \"Spotify\" --category tracks \"query\"`\n\nNotes\n- If SSDP fails, specify `--ip <speaker-ip>`.\n- Spotify Web API search is optional and requires `SPOTIFY_CLIENT_ID/SECRET`.""",
                examples=[],
                gotchas=[],
            ...
            tags=["skill", "clawdbot", "sonoscli", "cli"],
            version="1.0.0",
        )
