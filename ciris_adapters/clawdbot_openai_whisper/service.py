"""
OpenaiWhisper Tool Service for CIRIS.
...
class OpenaiWhisperToolService:
    """
    OpenaiWhisper tool service providing skill-based guidance.
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self._call_count = 0
        logger.info("OpenaiWhisperToolService initialized")

    async def start(self) -> None:
        """Start the service."""
        logger.info("OpenaiWhisperToolService started")

    async def stop(self) -> None:
        """Stop the service."""
        logger.info("OpenaiWhisperToolService stopped")
    ...
    def _build_tool_info(self) -> ToolInfo:
        ...
        return ToolInfo(
            ...
            category="skill",
            when_to_use="""When you need to local speech-to-text with the whisper cli (no api key).""",
            requirements=ToolRequirements(
                binaries=[
                    BinaryRequirement(name="whisper"),
                ],
            ),
            install_steps=[
                InstallStep(
            ...
            documentation=ToolDocumentation(
                quick_start="Use `whisper` to transcribe audio locally.",
                detailed_instructions="""# Whisper (CLI)\n\nUse `whisper` to transcribe audio locally.\n\nQuick start\n- `whisper /path/audio.mp3 --model medium --output_format txt --output_dir .`\n- `whisper /path/audio.m4a --task translate --output_format srt`\n\nNotes\n- Models download to `~/.cache/whisper` on first run.\n- `--model` defaults to `turbo` on this install.\n- Use smaller models for speed, larger for accuracy.""",
                examples=[],
                gotchas=[],
            ...
            tags=["skill", "clawdbot", "openai_whisper", "cli"],
            version="1.0.0",
        )
