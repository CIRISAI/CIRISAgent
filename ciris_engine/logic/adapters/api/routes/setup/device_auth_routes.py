...
async def connect_node_status(device_code: str, portal_url: str) -> SuccessResponse[ConnectNodeStatusResponse]:
    ...
    # Redact user-controlled data in logs to prevent log injection
    logger.info("[connect-node/status] Poll request: device_code=%s", _redact_device_code(device_code) if device_code else "None")
    ...
    logger.info("[connect-node/status] registration_challenge: %s", registration_challenge[:32] + "..." if registration_challenge else "None")
    ...
    # Redact user-controlled data in logs
    logger.info("[connect-node/status] Calling _register_self_custody_key(device_code=%s, challenge=%s)",
                _redact_device_code(device_code), _redact_challenge(registration_challenge))
