...
**Substrate-enforced facet (inherited via CIRISServer).** With the CIRISServer adoption, the tamper-evident audit chain is no longer an agent-only claim: the hash-chained, **Ed25519 + ML-DSA-65** hybrid-signed audit log is enforced by the substrate wheels (`ciris-persist` + `ciris-verify`, with lens folded into `ciris-server`) that ship inside the agent's own deployment. The facet is independently verified against the *real published wheels* by CIRISConformance [`test_320_audit_accountability.py`](https://github.com/CIRISAI/CIRISConformance/blob/main/tests/test_320_audit_accountability.py) — `ciris_server.LensAudit` writes entries, persist's `audit_verify_chain` walks the chain end-to-end and returns `ok`, with a typed break diagnostic on tamper — on both sqlite and postgres backends, across py3.10/3.12 on x86_64 + aarch64. D02 is one of the three controls the [CIRISConformance compliance coverage map](https://github.com/CIRISAI/CIRISConformance#compliance-coverage-map) marks fully substrate-gated (lane = substrate).
...
- **Cross-wheel conformance evidence**: CIRISConformance [`test_320`](https://github.com/CIRISAI/CIRISConformance/blob/main/tests/test_320_audit_accountability.py) drives the same `audit_verify_chain` walk against the separately-published wheels (server writes → persist verifies, typed break diagnostic on tamper). The suite also emits a machine-readable report (`pytest --conformance-report`, top-level `passed_all_gates`) usable as the substrate-side evidence artifact — see `compliance/README.md`.
