...
class CIRISVerify:
    ...
    def _load_library(self) -> None:
        ...
        # ciris_verify_version() -> *const c_char (static string)
        try:
            self._lib.ciris_verify_version.argtypes = []
            self._lib.ciris_verify_version.restype = ctypes.c_char_p
            self._has_version_support = True
        except AttributeError:
            self._has_version_support = False
    ...
    def get_version(self) -> str:
        ...
        Returns:
            Version string (e.g., "0.9.7") or "unknown" if not available.
        """
        if not getattr(self, "_has_version_support", False):
            return "unknown"

        try:
            version_ptr = self._lib.ciris_verify_version()
            if version_ptr:
