...
_event_loop = None
_runtime_ref = None  # Reference to CIRISRuntime for graceful shutdown
_watchdog_log = None
...
def watchdog_thread_func():
    ...
    while True:
        ...
        if check_restart_signal():
            ...
            # Graceful shutdown: shut down the runtime (releases port) THEN stop the loop
            if _event_loop is not None:
                try:
                    _watchdog_log.info("Scheduling graceful runtime shutdown...")

                    async def _graceful_shutdown():
                        """Shut down the server and release the port, then stop the loop."""
                        global _runtime_ref
                        try:
                            if _runtime_ref is not None:
                                _watchdog_log.info("Shutting down runtime (releasing port 8080)...")
                                await _runtime_ref.shutdown()
                                _runtime_ref = None
                                _watchdog_log.info("Runtime shutdown complete, port released")
                        except Exception as ex:
                            _watchdog_log.error(f"Runtime shutdown error: {ex}")
                        finally:
                            _event_loop.stop()
                    ...
                    _watchdog_log.error(f"Could not schedule shutdown: {e}, forcing loop.stop()")
                    try:
...
async def start_mobile_runtime_with_watchdog():
    ...
    # Store references for external watchdog thread
    ...
    try:
        ...
        try:
            ...
            runtime_log.warning(f"Shutdown error (may already be shut down by watchdog): {e}")
