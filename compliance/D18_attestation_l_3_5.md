...
This consumer-side-composition rule is formally specified by the federation: **CEG §8.1.9 Policy I — Attestation-Ladder Composition** (`CIRISRegistry/FSD/CEG/08_composition.md`). The on-wire attestation prefixes are mechanism-only — `self_verify` / `hardware_rooted` / `registry_consensus` / `license_validity` / `agent_integrity` — and the old `attestation:l{N}:*` level-prefixed form is deprecated (§5.2 namespace + §13.1 anti-pattern; the wire-break landed at CEG 0.2 and has held through **CEG 0.15**, the Agent 3.0 / CEG 1.0 candidate). The L1–L5 numbering survives only as a consumer-rendering convention; the wire never speaks levels. CIRIS 3.0 ships with CEG 0.15 as the federation wire format.

Under the **CEG-native agent** (CIRISAgent#840, `FSD/CEG_NATIVE_AGENT.md`), the integrity record this ladder attests over is itself CEG: at the local tier the agent's `graph_nodes` ARE self-level CEG self-attestations (`witness_relation: self`, signature deferred per §10.1.3 until federation promotion; tier model §10.1.5). The L4/L5 file-integrity and tamper-evident-history evidence an auditor reads at this dimension is therefore the same envelope the federation reads after promotion — one record, two audiences. The agent's conformance here is one of the four implementations gating CEG RC1 (with NodeCore / LensCore / Registry).
