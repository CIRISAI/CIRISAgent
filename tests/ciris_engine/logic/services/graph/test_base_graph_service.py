...
class TestBaseGraphServiceStoreOperations:
    ...
    async def test_store_in_graph_success(self, graph_service, memory_bus):
        ...
        memory_bus.memorize.return_value = MemoryOpResult(status=MemoryOpStatus.OK, data=node)
    ...
    async def test_store_in_graph_with_to_graph_node(self, graph_service, memory_bus):
        ...
        memory_bus.memorize.return_value = MemoryOpResult(status=MemoryOpStatus.OK, data=graph_node)
    ...
    async def test_store_in_graph_failure(self, graph_service, memory_bus):
        ...
        # Mock failed storage
        memory_bus.memorize.return_value = MemoryOpResult(status=MemoryOpStatus.ERROR, error="Storage failed")
...
class TestBaseGraphServiceQueryOperations:
    ...
    async def test_query_graph_success_with_list(self, graph_service, memory_bus):
        ...
        query = MemoryQuery(node_id="test_query", type=NodeType.CONCEPT, scope=GraphScope.LOCAL)
        ...
        memory_bus.recall.return_value = MemoryOpResult(status=MemoryOpStatus.OK, data=nodes)
    ...
    async def test_query_graph_success_with_single_node(self, graph_service, memory_bus):
        ...
        memory_bus.recall.return_value = MemoryOpResult(status=MemoryOpStatus.OK, data=node)
    ...
    async def test_query_graph_failure(self, graph_service, memory_bus):
        ...
        memory_bus.recall.return_value = MemoryOpResult(status=MemoryOpStatus.ERROR, error="Query failed")
...
class TestBaseGraphServiceEdgeCases:
    ...
    async def test_store_none_node(self, graph_service, memory_bus):
        ...
        memory_bus.memorize.return_value = MemoryOpResult(status=MemoryOpStatus.ERROR, error="Invalid node")
    ...
    async def test_query_with_none_query(self, graph_service, memory_bus):
        ...
        memory_bus.recall.return_value = MemoryOpResult(status=MemoryOpStatus.ERROR, error="Invalid query")
    ...
    async def test_concurrent_operations(self, graph_service, memory_bus):
        ...
        # Mock responses
        memory_bus.memorize.side_effect = [MemoryOpResult(status=MemoryOpStatus.OK, data=node) for node in nodes]
        memory_bus.recall.return_value = MemoryOpResult(status=MemoryOpStatus.OK, data=nodes)

        # Run concurrent operations
        store_tasks = [graph_service.store_in_graph(node) for node in nodes]
        query_task = graph_service.query_graph(
            MemoryQuery(node_id="test", scope=GraphScope.LOCAL, type=NodeType.CONCEPT)
        )
...
class TestBaseGraphServiceNewMethods:
    ...
    async def test_query_memory_delegates_to_recall(self, graph_service, memory_bus):
        ...
        query = MemoryQuery(node_id="legacy-123", scope=GraphScope.LOCAL)
    ...
    async def test_query_memory_delegates_to_search(self, graph_service, memory_bus):
        ...
        query = MemoryQuery(node_id="*", scope=GraphScope.LOCAL)  # Wildcard for search
...
class TestSchemaValidation:
    ...
    def test_all_required_schemas_exist(self):
        ...
        caps = ServiceCapabilities(service_name="test", actions=["test"], version="1.0.0")
        assert caps.service_name == "test"

        node = GraphNode(
            id="test", type=NodeType.CONCEPT, scope=GraphScope.LOCAL, attributes=GraphNodeAttributes(created_by="test")
        )
        assert node.id == "test"
