...
class MCPWiseService:
            ...
            if hasattr(client, "list_prompts"):
                ...
                for prompt in prompts:
                    ...
                    description = (
                        ...
                        else str(prompt.get("description", ""))
                    )
                    arguments = (
                        prompt.arguments
                        if hasattr(prompt, "arguments")
                        else prompt.get("arguments", [])
                    )
                    ...
                    if not allowed:
                        logger.warning(
                            f"Prompt '{name}' from server '{server_id}' blocked by security"
                        )
                        continue
    ...
    async def check_authorization(
        self, wa_id: str, action: str, resource: Optional[str] = None
    ) -> bool:
