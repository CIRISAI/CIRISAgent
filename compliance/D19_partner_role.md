...
**Adapter manifest as the runtime peer-partner record.** Per `CLAUDE.md` § "Adapter Development", every adapter ships a `manifest.json` declaring its capabilities. Adapters are the closest thing to a partner-role at runtime; reference implementations are at `ciris_adapters/sample_adapter/`, `ciris_adapters/home_assistant/`, and `ciris_adapters/wallet/` (the wallet example uses `dma_guidance=ToolDMAGuidance(requires_approval=True)` — **an advisory marker, not a gate**: nothing branches on the field, so this is a declaration of partner-role sensitivity rather than an enforced financial-tool control. See [CIRISAgent#942](https://github.com/CIRISAI/CIRISAgent/issues/942) and `FSD/THREAT_MODEL_2.9.7.md`. The enforced financial control is the budget envelope, `FSD/BUDGET_ENVELOPE.md`).
