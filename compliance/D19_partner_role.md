...
**Kinds of change (RATCHET 11+1)**: rules — Record: indirect

**Seed reference**: `SEED_DIMENSIONS.yaml` v1.1, dimension `D19` ([source](https://github.com/CIRISAI/ciris-response-magnifica-humanitas/blob/main/SEED_DIMENSIONS.yaml))
...
**Adapter manifest as the runtime peer-partner record.** Per `CLAUDE.md` § "Adapter Development", every adapter ships a `manifest.json` declaring its capabilities. Adapters are the closest thing to a partner-role at runtime; reference implementations are at `ciris_adapters/sample_adapter/`, `ciris_adapters/home_assistant/`, and `ciris_adapters/wallet/` (the wallet example uses `dma_guidance=ToolDMAGuidance(requires_approval=True)` — **a real gate since [CIRISAgent#942](https://github.com/CIRISAI/CIRISAgent/issues/942) landed** — this sentence previously read "an advisory marker, not a gate: nothing branches on the field", which was true when written and is now false. `ThoughtProcessor._enforce_tool_approval` (`logic/processors/core/thought_processor/main.py:257`) runs as Phase 4.9 and is fail-closed on every axis: no envelope, an issuer kind outside `APPROVAL_ISSUER_KINDS`, or the tool absent from `granted_tools` each deny, and the action is rewritten to DEFER for a human. It is placed after conscience on purpose, so a conscience cannot override an authorization denial. Still ungated is every *other* tool — the Phase-2 seam, [CIRISAgent#905](https://github.com/CIRISAI/CIRISAgent/issues/905). See `FSD/THREAT_MODEL_2.9.7.md`, noting it describes 2.9.7 against a repo now well past it. The enforced financial control is the budget envelope, `FSD/BUDGET_ENVELOPE.md`).
