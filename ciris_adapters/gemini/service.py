...
class GeminiToolService:
    """
    Gemini tool service providing skill-based guidance.
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self._call_count = 0
        logger.info("GeminiToolService initialized")

    async def start(self) -> None:
        """Start the service."""
        logger.info("GeminiToolService started")

    async def stop(self) -> None:
        """Stop the service."""
        logger.info("GeminiToolService stopped")
    ...
    def _build_tool_info(self) -> ToolInfo:
        ...
        return ToolInfo(
            ...
            category="skill",
            when_to_use="""When you need to gemini cli for one-shot q&a, summaries, and generation.""",
            requirements=ToolRequirements(
                binaries=[
                    BinaryRequirement(name="gemini"),
                ],
            ),
            install_steps=[
                InstallStep(
            ...
            documentation=ToolDocumentation(
                quick_start="Use Gemini in one-shot mode with a positional prompt (avoid interactive mode).",
                detailed_instructions="""# Gemini CLI\n\nUse Gemini in one-shot mode with a positional prompt (avoid interactive mode).\n\nQuick start\n- `gemini \"Answer this question...\"`\n- `gemini --model <name> \"Prompt...\"`\n- `gemini --output-format json \"Return JSON\"`\n\nExtensions\n- List: `gemini --list-extensions`\n- Manage: `gemini extensions <command>`\n\nNotes\n- If auth is required, run `gemini` once interactively and follow the login flow.\n- Avoid `--yolo` for safety.""",
                examples=[],
                gotchas=[],
