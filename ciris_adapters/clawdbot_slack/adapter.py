...
class SlackAdapter(Service):
    """
    Slack adapter for CIRIS.

    Provides tool guidance for: Use when you need to control Slack from Moltbot via the slack tool, including reacting to messages or pinning/unpinning items in Slack channels or DMs.

    Original Clawdbot skill: slack
    """
    ...
    def __init__(self, runtime: Any, context: Optional[Any] = None, **kwargs: Any) -> None:
        ...
        self.tool_service = SlackToolService(config=adapter_config)
    ...
    async def run_lifecycle(self, agent_task: Any) -> None:
        ...
        logger.info("Slack adapter lifecycle started")
        try:
            await agent_task
        except asyncio.CancelledError:
            logger.info("Slack adapter lifecycle cancelled")
        finally:
            await self.stop()
