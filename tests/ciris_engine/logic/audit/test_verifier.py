...
def populated_db(temp_db):
    ...
    for i in range(1, 11):
        ...
        """, (
            i, i, f"hash_{i}", f"hash_{i-1}" if i > 1 else "genesis",
            f"signature_{i}", "test_key", datetime.now(timezone.utc).isoformat(),
            "test_event", f"entity_{i}", "success"
        ))
...
class TestSchemaValidation:
    ...
    def test_all_required_schemas_exist(self):
        ...
        assert hasattr(CompleteVerificationResult, 'model_validate'), "CompleteVerificationResult is not a Pydantic model!"
        ...
        entry_result = EntryVerificationResult(
            valid=True,
            entry_id=1,
            hash_valid=True,
