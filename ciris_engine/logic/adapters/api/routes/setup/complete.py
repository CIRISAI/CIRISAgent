...
async def complete_setup(setup: SetupCompleteRequest, request: Request) -> SuccessResponse[Dict[str, str]]:
    ...
    try:
        ...
        # Auditing its initialization is best-effort — if CIRISVerify can't
        # produce a signing key (e.g. attestation failure, FFI issue), the
        # agent should still finish setup in a degraded-but-functional mode
        # rather than leaving the user locked out with only a half-written
        # .env file. The admin-user creation and runtime resume below MUST
        # still run so the operator can log in and retry attestation later.
        ...
        try:
            ...
            logger.warning(
                "[Setup Complete] Self-custody signing key unavailable (%s: %s) — "
                "continuing in degraded mode. Admin user will be created and the "
                "agent will start; signing-dependent features (WA attestation, "
                "audit signing) will re-initialize on the next restart once the "
                "underlying issue is resolved.",
                type(signing_err).__name__,
                signing_err,
            )
