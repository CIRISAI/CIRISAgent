...
from ciris_engine.schemas.services.filters_core import AdaptiveFilterConfig, FilterPriority, UserTrustProfile
...
class TestAnonymousFiltering:
    ...
    async def test_anti_gaming_rapid_switching(self, filter_service):
        ...
        assert await filter_service.handle_consent_transition(user_id, "temporary", "anonymous") is False
        assert (
            await filter_service.handle_consent_transition(user_id, "anonymous", "temporary") is True
        )  # 4th transition
