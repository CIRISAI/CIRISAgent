        ...
        try:
            ...
            for attempt in range(15):
                try:
                    ...
                    logger.warning(
                        f"[WALLET_INIT] has_key_sync raised {type(key_err).__name__} (attempt {attempt + 1}): {key_err}"
                    )
                _time.sleep(1)
                ...
                try:
                    ...
                    logger.warning(
                        f"[WALLET_INIT] has_key_sync raised {type(final_err).__name__} (final check): {final_err}"
                    )
            if not has_key:
