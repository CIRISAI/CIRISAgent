            ...
            if summary_node.type == NodeType.TSDB_SUMMARY:
                        ...
                        if self._is_metrics_config(config_node):
                            edges.append(
                                (
                                    ...
                                    config_node,
                                    "METRICS_CONFIG",
                                    {"period": period_label, "config_type": self._get_config_type(config_node)},
                                )
                            )
                ...
                if "social" in memory_nodes:
                    for social_node in memory_nodes["social"]:
                        edges.append((summary_node, social_node, "SOCIAL_CONTEXT", {"period": period_label}))
                ...
                if "behavioral" in memory_nodes:
                    for behavioral_node in memory_nodes["behavioral"]:
                        edges.append((summary_node, behavioral_node, "BEHAVIORAL_PATTERN", {"period": period_label}))
                    ...
                    for identity_node in memory_nodes["identity"]:
                        edges.append(
                            (
                                ...
                                identity_node,
                                "IDENTITY_AUDIT",
                                {"period": period_label, "change_type": self._get_identity_change_type(identity_node)},
                            )
                        )
                ...
                if "identity_snapshot" in memory_nodes:
                    for snapshot_node in memory_nodes["identity_snapshot"]:
                        edges.append((summary_node, snapshot_node, "IDENTITY_SNAPSHOT_REF", {"period": period_label}))
                ...
                if "concept" in memory_nodes:
                    for concept_node in memory_nodes["concept"]:
                        if self._is_task_relevant_concept(concept_node):
                            edges.append((summary_node, concept_node, "TASK_CONCEPT", {"period": period_label}))
