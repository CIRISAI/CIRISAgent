...
from .security import ClientSession, MCPServerSecurityManager
...
class Adapter(Service):
    ...
    def _initialize_config(self, kwargs: MCPServerAdapterKwargs) -> None:
        ...
        logger.info(
            f"MCP Server configured: id={self.config.server_id}, "
            f"transport={self.config.transport.type.value}"
        )
    ...
    async def _run_http_server(self) -> None:
        ...
        logger.info(
            f"Starting MCP HTTP server on "
            f"{self.config.transport.host}:{self.config.transport.port}"
        )
    ...
    async def _handle_initialize(
        ...
        server_caps = ServerCapabilities(
            ...
            resources={
                "subscribe": False,
                "listChanged": True,
            } if self.config.exposure.expose_resources else None,
