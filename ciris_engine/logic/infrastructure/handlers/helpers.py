...
def create_follow_up_thought(
    ...
    # Inherit preferred_language from parent — Task is record of truth, parent
    # already has the resolved value, propagate to keep the locale-coherent
    # chain through every follow-up generated by handlers.
    parent_lang = (
        getattr(parent, "preferred_language", None)
        or (getattr(parent.context, "preferred_language", None) if parent.context else None)
    )
    follow_up = Thought(
