...
against — a floor, not a pin (CIRISClient#39). Three forms: `>=X.Y.Z` (written against
release X.Y.Z), `>X.Y.Z` (*no released client at or below X.Y.Z carries the surface*), and
`unreleased` (*no release carries it at all* — the flow describes an unmerged client PR,
named in the CSD's **Origin**; every client is refused as "cannot start" until the CSD
names the release that ships it). Prefer `unreleased` over a numeric floor for an
unmerged PR: a version floor stops refusing the moment the next release is cut,
which turns the flow into a binding verdict against a client that still lacks the screens.
...
| `Client floor` absent or unparseable (`>=X.Y.Z` / `>X.Y.Z` / `unreleased`) | the flow cannot refuse an older client loudly |
